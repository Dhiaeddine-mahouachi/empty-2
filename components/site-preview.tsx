...
export function SitePreview({ project, draft = false, compact = false }: { project: ProjectInput; draft?: boolean; compact?: boolean }) {
  ...
  return (
    <div className={`generated-site site-${project.templateId} ${compact ? "is-compact" : ""}`} style={{ "--site-accent": project.primaryColor } as React.CSSProperties} dir={direction}>
      ...
      <div className="generated-nav"><b>{logo && <img className="generated-logo" src={logo} alt="" />}{project.businessName}</b><div className="generated-links" role="navigation" aria-label={project.language === "ar" ? "التنقل الرئيسي" : project.language === "en" ? "Main navigation" : "Ana menü"}><a href="#offers">{menuSite ? copy.menu : copy.offers}</a><a href="#about">{copy.about}</a><a href="#contact">{copy.contact}</a></div><a className="generated-cta" href={`tel:${project.phone}`}>{copy.reach}</a></div>
